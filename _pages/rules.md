...
- 오픈소스 모델(예: LLaMA 등)은 활용 가능하나, **2026년 5월 31일 이전**에 모델 가중치가 공개된 경우에 한함.
...
### **5. 예선 평가 규칙**
- 예선기간 중 리더보드는 전체 테스트 데이터의 70% (Public data)만을 이용하여 업데이트됨. 순위는 **Exact Match Accuracy**를 기준으로 결정.  
...
- 예선 종료 후 **상위 30위 이내** 참가자는 지정된 기한 내에 아래 자료를 제출해야 함.
    - 학습 코드 및 추론 코드 (.py 형태) 와 최종 모델 가중치 파일
    - 과제 해결을 위해 사용한 방법론을 보고서 형태로 정리 (A4 5쪽 이내 분량, PDF 파일로 제출하고 별도 양식은 없음.)
- 제출 코드 관련 준수 사항:
    - README.md 파일을 상세히 작성할 것 - 운영진이 결과를 재현할 수 있도록 필요한 실행 방법 및 설정 정보를 포함.
...
- 30위 이하 팀도 독창적인 아이디어를 제안한 경우 보고서와 코드를 제출할 수 있으며, 종합 평가 결과에 따라 본선 진출 후보로 선정될 수 있음.
- 상세한 내용은 Kaggle 내 공지 참조.
...
※ **규칙이 추가될 수 있으니 수시로 확인하시어 불이익을 받는 일이 없도록 해주시기 바랍니다.** (최종 업데이트: 7월 20일 17:00)